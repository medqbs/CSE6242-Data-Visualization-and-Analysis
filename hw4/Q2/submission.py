...
class Utility(object):
    ...
    def entropy(self, class_y):
        ...
        #############################################
        n = len(class_y)
        # edge case, empty label list
        if n == 0:
            return 0
        labels, counts = np.unique(class_y, return_counts=True)
        probs = counts / n
        calc = np.vectorize(lambda x: -x * np.log2(x))
        entropy = calc(probs).sum()
        #############################################
        return entropy
    ...
    def information_gain(self, previous_y, current_y):
        ...
        #############################################
        n = len(previous_y)
        E_prev = self.entropy(previous_y)
        E_curr = sum(list(map(lambda x: self.entropy(x)*len(x)/n, current_y)))
        info_gain = E_prev - E_curr
    ...
    def best_split(self, X, y):
       ...
       'y_right': [0, 0, 1]
        ...
        for i in X:
            f0.append(i[0])
            f1.append(i[1])
        for j in f0: #Get result with Attribute = 0
            X_left, X_right, y_left, y_right = self.partition_classes(X, y, 0, j)
            IGnew = self.information_gain(self, previous_y, [y_left, y_right])
            if IGnew > IG:
                best_split_val = j
                X_left1, X_right1, y_left1, y_right1 = _left, X_right, y_left, y_right
                best_split_val = 0
                IGB = IGnew
            IG = IGnew
        for j in f1: #Get result with Attribute = 1
            X_left, X_right, y_left, y_right = self.partition_classes(X, y, 1, j)
            IGnew = self.information_gain(self, previous_y, [y_left, y_right])
            if IGnew > IG:
                best_split_val = j
                X_left1, X_right1, y_left1, y_right1 = _left, X_right, y_left, y_right
                ...
                IGB = IGnew
            IG = IGnew
        res = {'split_attribute':split_attribute, 'split_val':split_val, 'X_left':X_left1, 'X_right':X_right1, 'y_left':y_left1, 'y_right':y_right1, 'info_gain':IGB}
        return res
...
class DecisionTree(object):
    ...
    def build_tree(self, X, y):
        if len(X) <= 15: # has less than 15 points in the node, stop splitting
            return [[-1, self.get_node_label(y), float("NaN"), float("NaN")]]
            # -1 in attr represents a leaf node

        if len(set(y)) == 1: # homogeneous y label, stop splitting
            return [[-1, y[0], float("NaN"), float("NaN")]]

        # Select random feature to split on
        num_feat = len(X[0])
        choices = list(range(num_feat))
        attr = np.random.choice(choices)

        # if selected attr homonegous -> select another till able to perform split
        while not self.check_valid_attr(X, attr):
            choices.remove(attr)
            if len(choices) == 0:
                return [[-1, self.get_node_label(y), float("NaN"), float("NaN")]]
            attr = np.random.choice(choices)

        SplitVal = self.get_split_val(X, y, attr)
        X_left, X_right, y_left, y_right = Utility().partition_classes(X, y, attr, SplitVal)

        # If split cause one side to be empty, stop and return
        if len(y_left) == 0:
            return [[-1, self.get_node_label(y_right), float("NaN"), float("NaN")]]
        if len(y_right) == 0:
            return [[-1, self.get_node_label(y_left), float("NaN"), float("NaN")]]

        lefttree = self.build_tree(X_left, y_left)
        righttree = self.build_tree(X_right, y_right)
        root = [[attr, SplitVal, 1, len(lefttree)+1]]
        return root + lefttree + righttree

    def check_valid_attr(self, X, attr):
        feat = set([x[attr] for x in X])
        return (len(feat) > 1)

    def get_split_val(self, X, y, attr):
        feat = list(set([x[attr] for x in X]))
        max_ig = -999
        split_val = None
        for x in feat:
            X_left, X_right, y_left, y_right = Utility().partition_classes(X, y, attr, x)
            curr_ig = Utility().information_gain(y, [y_left, y_right])
            if curr_ig > max_ig:
                max_ig = curr_ig
                split_val = x
        return split_val

    def get_node_label(self, y):
    ...
    def classify(self, record):
        ...
        while self.tree[i][0] != -1:
            ...
            feature = self.tree[i][0]
            if record[feature] <= SplitVal:
                i += self.tree[i][2]
            else:
                i += self.tree[i][3]
...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        ### Implement your code here
        #############################################
        ind = np.random.randint(len(XX), size=n)
        for i in ind:
