...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        for dictat in movie_Cast:
            #print(dictat)
            movie_Cast_shrinked = {key_i: dictat[key_i] for key_i in dictat.keys() & {'cast_id','character','credit_id',"order","name"}}
        ...
        if exclude_ids:
            ...
            mc_ID_filter = [d for d in movie_Cast_list if d['cast_id'] not in exclude_ids_ ]
            
        if limit:
            mc_order_filter = [d for d in mc_ID_filter if d["order"] < limit ]
        #print(mc_order_filter)
        return mc_order_filter
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        #movie_data1_list = json.loads(r2.read().decode())
        #movie_Credit = movie_data1_list['cast']
        #print("Yessss")
        for dictat1 in movie_Credit:
            #print(dictat)
            movie_Credit_shrinked = {key_i: dictat1[key_i] for key_i in dictat1.keys() & {'id','vote_average','title'}}
...
Actor_main_Graph.add_node('2975','Laurence Fishburne')
for i in Cast_details:
    ...
    for cast in movie_cast:
        Actor_main_Graph.add_node(cast['cast_id'], cast['name'])
        Actor_main_Graph.add_edge(cast['cast_id'], '2975')
...
if __name__ == "__main__":
    ...
    tmdb_api_utils = TMDBAPIUtils(api_key="c5b2d9b19dd2e779e735854fb4f2de79")
    tmdb_api_utils.get_movie_cast('75780', 5, [500])
    cre = tmdb_api_utils.get_movie_credits_for_person('2975', 8)
    Cast_details = tmdb_api_utils.get_movie_credits_for_person("2975", 8.0)
    #print(Cast_details)
    for i in Cast_details:
        movie_cast = tmdb_api_utils.get_movie_cast(i['id'], limit=3,exclude_ids=exclude_list)
